...
# 贝叶斯分类器
求P(c | x )
1. 先对联合概率分布P(Q,c)建模，依赖P(Q,c)以及贝叶斯公式获得P(c|x)，是一种“生成式模型"(generative models). 
    - 贝叶斯分类器
...
- 生成式模型
  - 建模目标联合概率分布 P(Y,X)
  - 核心思想 学习输入-输出的联合分布，然后通过贝叶斯公式计算条件概率 P(Y|X)
  - 朴素贝叶斯、隐马尔可夫模型（HMM）、高斯混合模型（GMM）、生成式对抗网络（GAN）
  - 理解数据的产生过程—— 它不仅能完成分类、回归等任务，还能生成新的、符合数据分布的样本。
- 判别式模型
  - 直接学习特征 X 和标签 Y 之间的映射关系f(x)，它不关心数据是如何生成的，只关心如何从 X 准确判断 Y。